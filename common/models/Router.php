...
class Router extends Model
{
	...
	public function bootstrap(){
		...
		if(strlen($url)>0 && !__IS_SUSPENDED__){
			if(__IS_ADMIN__){
				...
				$r = (new Dbf())->fetchOne("select a.item_id,a.item_type,a.route,a.url from slugs as a
	...
	protected function __registed_domain(){
		...
		// Get SID
		//$config = Yii::$app->session['config'];		 		
		$r = $this->db->fetchOne("select a.sid,b.code,a.is_admin,a.module,b.to_date
				from domain_pointer as a inner join shops as b on a.sid=b.id
		...
		define('__IS_SUSPENDED__',(new \Common\Models\Suspended)->checkSuspended());
	...
	protected function __get_domain_id(){
		...
		//view(\Yii::getAlias('@web'),true);
		$app_path = \Yii::getAlias('@app');
		$view_path = \Yii::getAlias('@module/views/'.__TEMP_NAME__);
		switch (\Yii::$device){
			case 'mobile':
				...
				break;
			default:
				$dir = \Yii::getAlias('@web') . '/themes' . DS  . __TEMP_NAME__;
		...
		define('__LIBS_PATH__',\Yii::getAlias('@libs_path'));
