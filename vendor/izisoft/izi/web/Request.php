 ...
 *
 * Request is configured as an application component in [[\izi\web\Application]] by default.
 * You can access that instance via `izi::$app->request`.
 *
 ...
 * (e.g. `http://www.iziframework.com`), null if can't be obtained from `$_SERVER` and wasn't set. See
 * [[getHostInfo()]] for security related notes on this property.
 * @property string|null $hostName Hostname part of the request URL (e.g. `www.iziframework.com`). This
 ...
 * @since 2.0
 */
class Request extends \izi\base\Request
{
    /**
     ...
     * When CSRF validation is enabled, forms submitted to an izi Web application must be originated
     * from the same application. If not, a 400 HTTP exception will be raised.
     *
     * Note, this feature requires that the user client accepts cookie. Also, to use this feature,
     * forms submitted via POST method must contain a hidden input whose name is specified by [[csrfParam]].
     * You may use [[\izi\helpers\Html::beginForm()]] to generate his hidden input.
     *
     * In JavaScript, you may get the values of [[csrfParam]] and [[csrfToken]] via `izi.getCsrfParam()` and
     * `izi.getCsrfToken()`, respectively. The [[\izi\web\iziAsset]] asset must be registered.
     * You also need to include CSRF meta tags in your pages by using [[\izi\helpers\Html::csrfMetaTags()]].
     *
     ...
     * corresponding configurations for [[Izi::::createObject|creating the parser objects]].
     ...
     * ```
     * [
     *     'application/json' => 'izi\web\JsonParser',
     * ]
     * ```
     *
    ...
    {
        $result = Izi::$app->getUrlManager()->parseRequest($this);
        if ($result !== false) {
            ...
            throw new NotFoundHttpException(Izi::t('izi', 'Page not found.'));
        }
    }
     ...
     * @throws \izi\base\InvalidConfigException if a registered parser does not implement the [[RequestParserInterface]].
     * @see getMethod()
     * @see getBodyParam()
     * @see setBodyParams()
     */
    public function getBodyParams()
    {
        if ($this->_bodyParams === null) {
            ...
            if (isset($this->parsers[$contentType])) {
                $parser = Izi::createObject($this->parsers[$contentType]);
                if (!($parser instanceof RequestParserInterface)) {
                    throw new InvalidConfigException("The '$contentType' request parser is invalid. It must implement the izi\\web\\RequestParserInterface.");
                }
                $this->_bodyParams = $parser->parse($this->getRawBody(), $rawContentType);
            } elseif (isset($this->parsers['*'])) {
                $parser = Izi::createObject($this->parsers['*']);
                if (!($parser instanceof RequestParserInterface)) {
                    throw new InvalidConfigException("The fallback request parser is invalid. It must implement the izi\\web\\RequestParserInterface.");
                }
                $this->_bodyParams = $parser->parse($this->getRawBody(), $rawContentType);
     ...
     * > If you don't have access to the server configuration, you can setup [[\izi\filters\HostControl]] filter at
     * > application level in order to protect against such kind of attack.
     *
     * @property string|null schema and hostname part (with port number if needed) of the request URL
     * (e.g. `http://www.iziframework.com`), null if can't be obtained from `$_SERVER` and wasn't set.
     ...
     *
     * @return string|null hostname part of the request URL (e.g. `www.iziframework.com`)
     * @see getHostInfo()
     * @since 2.0.10
     */
    ...
    protected function loadCookies()
    {
        ...
        if ($this->enableCookieValidation) {
            ...
            foreach ($_COOKIE as $name => $value) {
                ...
                $data = Izi::$app->getSecurity()->validateData($value, $this->cookieValidationKey);
                if ($data === false) {
                    continue;
                }
    ...
    protected function loadCsrfToken()
    {
        if ($this->enableCsrfCookie) {
            ...
            return Izi::$app->getSession()->get($this->csrfParam);
        }
    }
    ...
    {
        $token = Izi::$app->getSecurity()->generateRandomString();
        if ($this->enableCsrfCookie) {
            $cookie = $this->createCsrfCookie($token);
            Izi::$app->getResponse()->getCookies()->add($cookie);
        } else {
            Izi::$app->getSession()->set($this->csrfParam, $token);
        }
        return $token;
    }
