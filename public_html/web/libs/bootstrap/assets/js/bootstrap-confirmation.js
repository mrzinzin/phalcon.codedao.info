 ...
 /* CONFIRMATION PUBLIC CLASS DEFINITION
	* =============================== */
	...
	var Confirmation = function (element, options) {
		...
		jQuery(element).on('show', function(e) {
			var options = that.options;
			var all = options.all_selector;
			if(options.singleton) {
				jQuery(all).not(that.$element).confirmation('hide');
			}
		});

		jQuery(element).on('shown', function(e) {
			var options = that.options;
			var all = options.all_selector;
			jQuery(this).next('.popover').one('click.dismiss.confirmation', '[data-dismiss="confirmation"]', $.proxy(that.hide, that))
			if(that.isPopout()) {
				if(!event_body) {
					event_body = jQuery('body').on('click', function (e) {
						if(jQuery(all).is(e.target)) return;
						if(jQuery(all).has(e.target).length) return;
						if(jQuery(all).next('div').has(e.target).length) return;

						jQuery(all).confirmation('hide');
	...
	Confirmation.prototype = $.extend({}, $.fn.tooltip.Constructor.prototype, {
		...
		, setContent: function () {
				...
				var $tip = this.tip()
					...
					, o = this.options,
					$data = getAttributes(this.$element)				
				$tip.find('.popover-title').text(title)
				jQuery.each($data,function(id,ed){
					$btnOk.attr('data-' + id ,ed);
				})
				 
				$btnOk.addClass(btnOkClass).html(btnOkLabel)
				//.attr('data-table',table).attr('href', href).attr('target', target)
				.on('click', o.onConfirm)
		...
		, getTable: function () {
				var table
					...
					table = $e.attr('data-table') //|| (typeof o.href == 'function' ? o.href.call($e[0]) :	o.href)
