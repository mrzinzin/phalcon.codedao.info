...
class Module extends ServiceLocator
{
    ...
    public static function getInstance()
    {
        ...
        return isset(Izi::$app->loadedModules[$class]) ? Izi::$app->loadedModules[$class] : null;
    }
    ...
    {
        if ($instance === null) {
            unset(Izi::$app->loadedModules[get_called_class()]);
        } else {
            Izi::$app->loadedModules[get_class($instance)] = $instance;
        }
    }
    ...
    {
        return Izi::getAlias('@' . str_replace('\\', '/', $this->controllerNamespace));
    }
    ...
    {
        $this->_viewPath = Izi::getAlias($path);
    }
    ...
    {
        $this->_layoutPath = Izi::getAlias($path);
    }
     ...
     * This method calls [[Izi::setAlias()]] to register the path aliases.
    ...
    public function getModule($id, $load = true)
    {
        ...
        if (isset($this->_modules[$id])) {
            if ($this->_modules[$id] instanceof Module) {
                ...
                Izi::trace("Loading module: $id", __METHOD__);
                /* @var $module Module */
                $module = Izi::createObject($this->_modules[$id], [$id, $this]);
     ...
     * array that can be used to create the module. In the latter case, [[Izi::createObject()]]
    ...
    public function runAction($route, $params = [])
    {
        ...
        if (is_array($parts)) {
            ...
            $oldController = Izi::$app->controller;
            Izi::$app->controller = $controller;
            $result = $controller->runAction($actionID, $params);
            if ($oldController !== null) {
                Izi::$app->controller = $oldController;
            }
    ...
    public function createController($route)
    {
        ...
        if (isset($this->controllerMap[$id])) {
            $controller = Izi::createObject($this->controllerMap[$id], [$id, $this]);
    ...
    public function createControllerByID($id)
    {
        ...
        if (is_subclass_of($className, 'izi\base\Controller')) {
            $controller = Izi::createObject($className, [$id, $this]);
            return get_class($controller) === $className ? $controller : null;
        } elseif (IZI_DEBUG) {
            throw new InvalidConfigException("Controller class must extend from \\izi\\base\\Controller.");
        }
        return null;
    }
